...
function f($info,$parameter) {
    global $config,$desktopua,$ch,$type;
    ...
    if(is_array($json["text"])) {
        foreach ($json["text"] as $v) {
            if($v["id"] != "-1") {
                $info["list"][] = array(
                    "id"   => $info["url"] = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://").$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']."?url=https://www.lanzoui.com/".$v["id"]."&type=".($type === "downAll" ? "down" : $type),  // ID
                    "ad"   => (bool)$v["t"], // 推广文件
        ...
        if (count($json['text']) != 0) $info["url"] = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://").$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']."?url=https://www.lanzoui.com/".$json['text'][0]["id"]."&type=down";
